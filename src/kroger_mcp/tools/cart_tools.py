...
DEFAULT_MODALITY = "DELIVERY"


def _normalize_modality(modality: str = None) -> str:
    """Normalize and validate the Kroger cart fulfillment modality."""
    normalized = (modality or DEFAULT_MODALITY).upper()
    if normalized not in VALID_MODALITIES:
        raise ValueError(
            f"Unsupported modality '{modality}'. Expected PICKUP or DELIVERY."
        )
    return normalized
...
def register_tools(mcp):
    ...
    async def add_items_to_cart(
        ...
        DELIVERY is the default. Pass PICKUP explicitly only when the user has
        requested pickup.

        Args:
            product_id: The product ID or UPC to add to cart
            quantity: Quantity to add (default: 1)
            modality: Fulfillment method - DELIVERY (default) or PICKUP
    ...
    async def bulk_add_to_cart(
        ...
        DELIVERY is the default for each item. Pass PICKUP explicitly on an item
        only when the user has requested pickup.

        Args:
            items: List of items to add. Each item should have:
                   - product_id: The product ID or UPC
                   - quantity: Quantity to add (default: 1)
                   - modality: DELIVERY (default) or PICKUP
        ...
        try:
            ...
            # Normalize once and use the same values for both Kroger and local tracking.
            normalized_items = []
            cart_items = []
            for item in items:
                ...
                cart_items.append({
                    "upc": normalized_item["product_id"],
